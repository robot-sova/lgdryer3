...
(function () {
  ...
  function init() {
    ...
    if (!isMobile()) {
      // На десктопе сбрасываем только при клике на интерактивный элемент
      // Скролл и движение мыши НЕ сбрасывают таймер
    }
    initTipsCanvas();
    resetIdleTimer();
  }

  // Создаём отдельный fixed canvas для советов (десктоп)
  let tipsCanvas = null;
  let tipsCtx = null;

  function initTipsCanvas() {
    if (isMobile()) return;
    tipsCanvas = document.createElement('canvas');
    tipsCanvas.id = 'dryerTipsCanvas';
    tipsCanvas.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:999;display:none;';
    document.body.appendChild(tipsCanvas);
    tipsCtx = tipsCanvas.getContext('2d');
    resizeTipsCanvas();
    window.addEventListener('resize', resizeTipsCanvas, { passive: true });
  }

  function resizeTipsCanvas() {
    if (!tipsCanvas) return;
    tipsCanvas.width = window.innerWidth;
    tipsCanvas.height = window.innerHeight;
  }
  ...
  function drawDesktopTipFixed() {
    if (!tipVisible || tipOpacity <= 0) return;
    const tip = TIPS[tipIndex];
    const cW = tipsCanvas.width;
    const cH = tipsCanvas.height;

    // Позиция советов — верхняя треть экрана, слева и справа от центра
    const boxW = cW * 0.18;
    const boxH = cH * 0.208;
    const pad  = cW * 0.018;
    const centerY = cH * 0.13;
    ...
    tipsCtx.save();
    tipsCtx.globalAlpha = tipOpacity;

    tipsCtx.fillStyle = 'rgba(10, 20, 40, 0.72)';
    tipsCtx.strokeStyle = 'rgba(100, 160, 255, 0.25)';
    tipsCtx.lineWidth = 1;
    tipsCtx.beginPath();
    tipsCtx.roundRect(boxX, boxY, boxW, boxH, 10);
    tipsCtx.fill();
    tipsCtx.stroke();

    tipsCtx.fillStyle = 'rgba(26, 108, 246, 0.7)';
    tipsCtx.beginPath();
    tipsCtx.roundRect(boxX, boxY, boxW, 2.5, [10, 10, 0, 0]);
    tipsCtx.fill();

    tipsCtx.fillStyle = '#4ae8ff';
    tipsCtx.font = `bold ${Math.round(cW * 0.014)}px system-ui, sans-serif`;
    tipsCtx.textAlign = 'left';
    tipsCtx.textBaseline = 'top';
    tipsCtx.fillText(`💡 Tip #${tipIndex + 1}`, boxX + pad, boxY + pad * 0.7);

    tipsCtx.fillStyle = 'rgba(200, 220, 255, 0.92)';
    tipsCtx.font = `${Math.round(cW * 0.011)}px system-ui, sans-serif`;
    tipsCtx.textBaseline = 'top';
    ...
    words.forEach(word => {
      const test = line ? line + ' ' + word : word;
      if (tipsCtx.measureText(test).width > maxWidth && line) {
        tipsCtx.fillText(line, boxX + pad, lineY);
        line = word;
        lineY += lineH;
      } else {
        line = test;
    ...
    if (line) tipsCtx.fillText(line, boxX + pad, lineY);
  ...
  function loop(ts) {
    ...
    // Основной canvas (hero) — рисуем только если hero виден
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    if (heroVisible) {
      const m = map();
      drawPowerBtn(m);
    ...
    // Tips canvas fixed — всегда рисуем советы в phase 3
    if (phase >= 3 && tipsCanvas && tipsCtx) {
      tipsCanvas.style.display = 'block';
      tipsCtx.clearRect(0, 0, tipsCanvas.width, tipsCanvas.height);
      drawDesktopTipFixed();
    }
